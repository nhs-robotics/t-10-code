...
public class OdometryLocalizer {
	...
	/**
	 * <p>Updates the underlying state of this odometry localizer. Calling this method more regularly increases accuracy of the calculations, but obviously it is impossible to call this method infinitely fast, so calculations will contain some error.</p>
	 * <p>Adapted from <a href="https://gm0.org/en/latest/docs/software/concepts/odometry.html">gm0</a>.</p>
	 */
	public void update() {
	...
	/**
	 * Field-centric coordinates maintain absolute position relative to the field. This means:
	 * <ul>
	 *     <li>X and Y coordinates are transformed based on total rotation</li>
	 *     <li>Heading remains the same as robot-centric pose</li>
	 * </ul>
	 * For example, if the robot:
	 * <ol>
	 *     <li>Moves forward 10 inches (facing 0°)</li>
	 *     <li>Turns 90° right</li>
	 *     <li>Moves forward 10 inches</li>
	 * </ol>
	...
	public MovementVector changeToRobotCenteredVelocity(double lateral, double horizontal) {
		return OdometryUtils.changeToRobotCenteredVelocity(new MovementVector(lateral, horizontal, 0, AngleUnit.DEGREES), getFieldCentricPose());
	}
}
